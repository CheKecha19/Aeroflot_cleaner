...
from config import USE_DIADOC_API, USE_SFERA_API
from datetime import datetime, timedelta
import logging

# Импорты API-клиентов
from api_clients.diadoc_api_client import get_diadoc_client as get_kontur_client
from api_clients.sfera_api_client import get_sfera_client
...
def get_kontur_file():
    """Находит файл Контур Диадок"""
    return find_latest_file(KONTUR_DIR)

def get_diadoc_file():
    """Находит файл Сфера Курьер"""
...
# ========== ФУНКЦИИ ЗАГРУЗКИ ИЗ EXCEL (СТАРЫЕ, ПОЛНОСТЬЮ РАБОЧИЕ) ==========

def load_kontur_data_from_excel():
    """Загрузка данных из Excel-файла Контур Диадок (старый метод)"""
    try:
        ...
        # Если переименование не сработало, ищем похожие колонки
        ...
        logger.info(f"Загружено {len(result_df)} записей из Контур Диадок (Excel)")
        return result_df
    except Exception as e:
        logger.error(f"Ошибка при загрузке данных Контур Диадок из Excel: {e}")
        return pd.DataFrame(columns=['Контур_Диадок_ФИО', 'Контур_Диадок_Администратор', 'Контур_Диадок_статус'])

def load_diadoc_data_from_excel():
    """Загрузка данных из Excel-файла Сфера Курьер (старый метод)"""
    try:
        ...
        # Поиск колонок, если стандартные имена не сработали
        ...
        # Берем только нужные колонки
        result_df = df[['Сфера_Курьер_ФИО', 'Сфера_Курьер_Активен', 'Сфера_Курьер_Администратор']].copy()
        # Преобразование значений в строки и обрезка пробелов
        if 'Сфера_Курьер_Активен' in result_df.columns:
            result_df['Сфера_Курьер_Активен'] = result_df['Сфера_Курьер_Активен'].astype(str).str.strip()
        if 'Сфера_Курьер_Администратор' in result_df.columns:
            result_df['Сфера_Курьер_Администратор'] = result_df['Сфера_Курьер_Администратор'].astype(str).str.strip()
        logger.info(f"Загружено {len(result_df)} записей из Сфера Курьер (Excel)")
        return result_df
    except Exception as e:
        logger.error(f"Ошибка при загрузке данных Сфера Курьер из Excel: {e}")
        return pd.DataFrame(columns=['Сфера_Курьер_ФИО', 'Сфера_Курьер_Активен', 'Сфера_Курьер_Администратор'])
...
# ========== НОВЫЕ ФУНКЦИИ-ДИСПЕТЧЕРЫ ==========

def get_kontur_data_source():
    """
    Возвращает DataFrame с данными Контур.Диадок.
    Источник: API (если USE_DIADOC_API = True) или Excel (старый метод).
    В режиме API при любой ошибке выбрасывает исключение.
    """
    if USE_DIADOC_API:
        logger.info("Режим API Контур.Диадок включён (USE_DIADOC_API = True)")
        client = get_kontur_client()
        if client:
            try:
                employees_raw = client.get_all_employees()
                df = client.employees_to_dataframe(employees_raw)
                if not df.empty:
                    logger.info(f"Данные успешно загружены через API: {len(df)} сотрудников")
                    return df
                else:
                    if USE_DIADOC_API:
                        raise RuntimeError("API Контур.Диадок вернул пустой список сотрудников")
            except Exception as e:
                logger.error(f"Ошибка при загрузке данных через API Контур.Диадок: {e}")
                if USE_DIADOC_API:
                    raise RuntimeError(f"Не удалось получить данные из API Диадок: {e}")
        else:
            if USE_DIADOC_API:
                raise RuntimeError("Клиент API Контур.Диадок не создан (проверьте DIADOC_API_KEY)")
    # Fallback на Excel
    logger.info("Загрузка данных Контур.Диадок из Excel (fallback)")
    return load_kontur_data_from_excel()

def get_diadoc_data_source():
    """
    Возвращает DataFrame с данными Сфера Курьер.
    Источник: API (если USE_SFERA_API = True) или Excel (старый метод).
    В режиме API при любой ошибке выбрасывает исключение.
    """
    if USE_SFERA_API:
        logger.info("Режим API Сфера Курьер включён (USE_SFERA_API = True)")
        client = get_sfera_client()
        if client:
            try:
                users_raw = client.get_user_list()
                df = client.userlist_to_dataframe(users_raw)
                if not df.empty:
                    logger.info(f"Данные успешно загружены через API Сфера: {len(df)} сотрудников")
                    return df
                else:
                    if USE_SFERA_API:
                        raise RuntimeError("API Сфера Курьер вернул пустой список сотрудников")
            except Exception as e:
                logger.error(f"Ошибка при загрузке данных через API Сфера: {e}")
                if USE_SFERA_API:
                    raise RuntimeError(f"Не удалось получить данные из API Сфера: {e}")
        else:
            if USE_SFERA_API:
                raise RuntimeError("Клиент API Сфера Курьер не создан (проверьте SFERA_USERNAME/SFERA_PASSWORD)")
    # Fallback на Excel
    logger.info("Загрузка данных Сфера Курьер из Excel (fallback)")
    return load_diadoc_data_from_excel()
